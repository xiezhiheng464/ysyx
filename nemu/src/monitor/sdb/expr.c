...
int64_t eval(int left,int right){
  int count_k=0;
  int kh_mark=1;
 // 判断左右符号是否匹配
  ...
  {	
    ...
    return eval(left + 1, right - 1);
	  ...
	  if(tokens[index_f].type=='+')return eval(left,index_f-1)+eval(index_f+1,right);
		  ...
		  int64_t k=eval(index_f+1,right);
		  if(k==0){
	  ...
	  else if(tokens[index_f].type==TK_REM) return vaddr_read(eval(index_f+1,right),4);
